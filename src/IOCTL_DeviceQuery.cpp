...
char *DiskInfo::strRemov(char* dst, const char* src, char ch)
{
	int i = -1, j = 0;
	while (src[++i])
		if (src[i] != ch)
         ...
         {         
            ...
            //硬盘编号为字母、数字和空格，但不是纯空格
			 if(isAlnumAndSpace && !isAllSpace){
				strRemov(serialNumber, serialNumber);
...
int DiskInfo::ReadDrivePortsInWin9X (void)
{
   ...
   VxDHandle = CreateFile (TEXT("\\\\.\\IDE21201.VXD"), 0, 0, 0,
							0, FILE_FLAG_DELETE_ON_CLOSE, 0);
...
int DiskInfo::ReadIdeDriveAsScsiDriveInNT (void)
{
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      TCHAR   driveName [256];
         ...
         //  Try to get a handle to PhysicalDrive IOCTL, report failure
         //  and exit if can't.
	  wsprintf (driveName, TEXT("\\\\.\\Scsi%d:"), controller);
	   ...
	   if (version.dwPlatformId == VER_PLATFORM_WIN32_NT)
		{
			...
			//cout << "ReadPhysicalDriveInNTWithAdminRights" << done << endl;
				//  this should work in WinNT or Win2K if previous did not work
				//  this is kind of a backdoor via the SCSI mini port driver into
				//     the IDE drives
			//done = ReadIdeDriveAsScsiDriveInNT ();
			//cout << "ReadIdeDriveAsScsiDriveInNT" << done << endl;
